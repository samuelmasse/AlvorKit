    ...
    IMockInterceptionRouteLifecycle
{
    private const string RouteId =
        "ProfiledConstructionFactoryCaller.Selected::newobj";
    ...
    /// <summary>Emits the original newobj delegate and installs an inert rewritten caller.</summary>
        ...
        MockInterceptionRoute value)
    {
        ...
        MethodInfo selected = Caller(
            nameof(ProfiledConstructionFactoryCaller.Selected));
        MethodInfo gateway =
            typeof(ProfiledConstructionFactoryRoute).GetMethod(
                nameof(ProfiledConstructionFactoryRoute.Invoke),
                BindingFlags.NonPublic | BindingFlags.Static)!;
        ConstructorInfo constructor =
            typeof(ProfiledConstructionFactoryTarget).GetConstructor(
                [typeof(int)])!;
        int operationOffset =
        ...
        var original =
            LoadedConstructionOriginalDelegate.Create<
                ProfiledConstructionFactoryOperation>(constructor);
        var wrapper =
    ...
    private static MethodInfo Caller(string name) =>
        typeof(ProfiledConstructionFactoryCaller).GetMethod(
            name,
            BindingFlags.NonPublic | BindingFlags.Static)!;

    private static void DriveCaller() =>
        _ = ProfiledConstructionFactoryCaller.Selected(1);
}
