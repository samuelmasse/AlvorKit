...
public sealed class CoverageOptionsTest
{
    ...
    {
        var options = CoverageOptions.Parse(["--binding", "glfw", "--binding", "native/freetype/conf/bindgen.yml"]);

        CollectionAssert.AreEqual(new[] { "glfw", "native/freetype/conf/bindgen.yml" }, options.BindingFilters.ToArray());
    }
