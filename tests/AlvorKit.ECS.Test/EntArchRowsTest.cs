...
public sealed class EntArchRowsTest
{
    ...
    public void ArchetypalRows_InitialAndFirstGrowth_KeepParallelCapacitiesAligned()
    {
        ...
        for (int i = 0; i < ents.Length; i++)
        {
            ...
            if (i == 0)
            {
                ...
                Assert.AreEqual(4, EntArchRows<CapacityArch>.CapacityAt(singletonLoc.AllocId, singletonLoc.ArchId));
                Assert.AreEqual(
                    4,
    ...
    public void ArchetypalArrayPool_ExactType_IsSharedAcrossFieldsAndGroups()
    {
        ...
        SharedPoolValue[] firstValues = EntArchColumn<SharedPoolValue, FirstField, FirstSharedPoolArch>
            .Values[firstLoc.AllocId][firstLoc.ArchId];
        ...
        SharedPoolValue[] secondValues = EntArchColumn<SharedPoolValue, SecondField, SecondSharedPoolArch>
            .Values[secondLoc.AllocId][secondLoc.ArchId];
    ...
    public void ArchetypalRows_SparseUsage_HalvesCapacityAndReturnsEmptyBuffers()
    {
        ...
        int allocId = pairLoc.AllocId;
        int archId = pairLoc.ArchId;
        Assert.AreEqual(32, EntArchRows<ShrinkArch>.CapacityAt(allocId, archId));

        for (int i = 0; i < 9; i++)
            Assert.IsTrue(ents[i].UnsetArchetypal<int, SecondField, ShrinkArch>());

        Assert.AreEqual(8, EntArchRows<ShrinkArch>.CountAt(allocId, archId));
        Assert.AreEqual(32, EntArchRows<ShrinkArch>.CapacityAt(allocId, archId));
        int[] firstBeforeShrink = EntArchColumn<int, FirstField, ShrinkArch>.Values[allocId][archId];
        int[] secondBeforeShrink = EntArchColumn<int, SecondField, ShrinkArch>.Values[allocId][archId];

        Assert.IsTrue(ents[9].UnsetArchetypal<int, SecondField, ShrinkArch>());

        Assert.AreEqual(7, EntArchRows<ShrinkArch>.CountAt(allocId, archId));
        Assert.AreEqual(16, EntArchRows<ShrinkArch>.CapacityAt(allocId, archId));
        int[] firstAfterShrink = EntArchColumn<int, FirstField, ShrinkArch>.Values[allocId][archId];
        int[] secondAfterShrink = EntArchColumn<int, SecondField, ShrinkArch>.Values[allocId][archId];
        ...
        Assert.AreEqual(0, EntArchRows<ShrinkArch>.CountAt(allocId, archId));
        Assert.AreEqual(0, EntArchRows<ShrinkArch>.CapacityAt(allocId, archId));
        Assert.IsNull(EntArchColumn<int, FirstField, ShrinkArch>.Values[allocId][archId]);
        Assert.IsNull(EntArchColumn<int, SecondField, ShrinkArch>.Values[allocId][archId]);
        ...
        Assert.AreEqual(4, EntArchRows<ShrinkArch>.CapacityAt(allocId, archId));
    ...
    public void ArchetypalRows_EmptyReferenceColumn_ClearsBufferBeforeReuse()
    {
        ...
        Assert.IsNull(EntArchColumn<PooledBox, FirstField, ReferencePoolArch>.Values[loc.AllocId][loc.ArchId]);
        ...
        PooledBox[] values = EntArchColumn<PooledBox, FirstField, ReferencePoolArch>
            .Values[reactivatedLoc.AllocId][reactivatedLoc.ArchId];
    ...
    private static void AssertPairCapacity(EntMut ent, int capacity)
    {
        ...
        Assert.AreEqual(capacity, EntArchRows<CapacityArch>.CapacityAt(loc.AllocId, loc.ArchId));
