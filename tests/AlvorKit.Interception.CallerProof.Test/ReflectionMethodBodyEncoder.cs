...
internal static class ReflectionMethodBodyEncoder
{
    ...
    internal static InterceptionMethodBody Read(MethodInfo method)
    {
        var body = method.GetMethodBody() ??
            throw new InvalidOperationException("The template method has no IL body.");
        if (body.ExceptionHandlingClauses.Count != 0)
        {
            throw new NotSupportedException(
                "The caller-bridge proof template cannot contain exception regions.");
        }

        var il = body.GetILAsByteArray() ??
            throw new InvalidOperationException("The template IL body is unavailable.");
        ...
        if ((flags & MoreSections) != 0)
            throw new InvalidOperationException("Unexpected extra method sections.");
