...
/// <summary>Retains an epoch-cleared mapping from unsigned 64-bit keys to caller-owned dense slots.</summary>
/// <param name="capacity">The active mapping count for which storage is initially retained.</param>
public class EpochIndex64(int capacity)
{
    private ulong[] keys = CreateArray<ulong>(capacity);
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public bool TryGet(ulong key, out int slot)
    {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public int GetOrAdd(ulong key, int slot, out bool added)
    {
        var index = Find(key, out var found);
        if (found)
        {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private int Find(ulong key, out bool found)
    {
        if (stamps.Length == 0)
        {
            found = false;
