...
/// <summary>Applies to 2x4 matrix types.</summary>
...
public interface IMat2x4<TSelf, TScalar, TColumn, TRow, TTranspose> :
    IMatNumeric<TSelf, TScalar, TColumn, TRow, TTranspose>
    where TSelf : struct, IMat2x4<TSelf, TScalar, TColumn, TRow, TTranspose>
    ...
    where TTranspose : struct
{
    ...
    static abstract TSelf CreateColumns(TColumn column0, TColumn column1);

    /// <summary>Creates a matrix from row vectors.</summary>
    static abstract TSelf CreateRows(TRow row0, TRow row1, TRow row2, TRow row3);
}
