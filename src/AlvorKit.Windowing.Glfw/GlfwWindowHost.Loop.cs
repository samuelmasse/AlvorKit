...
public partial class GlfwWindowHost
{
    ...
    private void RegisterCallbacks()
    {
        ...
        Glfw.SetWindowPosCallback(Window, (_, x, y) => OnMove(new(new Vec2i(x, y))));
        Glfw.SetFramebufferSizeCallback(Window, (_, width, height) => OnResize(new(new Vec2u(checked((uint)width), checked((uint)height)))));
