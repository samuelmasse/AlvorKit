...
the generator finds the current Git checkout. Explicit roots also support
source archives and newly scaffolded directories before `git init`.
The generator requires the .NET 10 SDK and bootstraps from its own project.
...
Root projects are `.csproj` files at the repository root or recursively under
`src`, `lib`, `scripts`, `tests`, `demos`, and `bench`. Discovery skips symlinks,
`.git`, `.vs`, `bin`, `obj`, `out`, `tmp`, `dist`, `node_modules`, and `packages`.
...
Repository additions and directory moves trigger rediscovery. Project, props,
targets, XML, SDK configuration, and directory changes trigger debounced full
graph reconciliation. Ordinary C# edits and build-output churn are ignored.
Periodic reconciliation every two minutes also covers missed notifications
and imports outside the observed directories. A singleton lock prevents two
watchers from owning the same directory. A failed OS watch exits with an error;
an overflow invalidates and reevaluates the graph.

An invalid graph is reported explicitly and its previous generated solution is
removed. Correcting the input regenerates it. The watcher never invents missing
references or retains stale output as a successful result. Moving a project
still requires updating its authored project references.
...
areas, so a game cannot format sibling engine files. Game CI runs its test
projects explicitly to keep engine tests out of the game's test suite. Direct
`dotnet test <solution>` runs every test project included in that solution.
