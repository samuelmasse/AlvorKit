...
Unlike the other AlvorKit native libraries there is no `.Native` runtimes package - the OpenGL implementation ships with the platform's graphics driver. `GlBackend.Load` resolves every entry point at runtime through a proc loader such as `Rgfw.GetProcAddressOpenGL`, after the GL context is current.
