...
# Output: native/freetype/runtimes/osx-<arch>/native/libfreetype.dylib
set -euo pipefail
...
cp "$(cd "$WORK_DIR/build-osx-$ARCH" && readlink -f libfreetype.dylib)" "$OUT_DIR/libfreetype.dylib"
strip -x "$OUT_DIR/libfreetype.dylib"

# Verify: arm64, Apple system libraries only expected.
file "$OUT_DIR/libfreetype.dylib" | grep -q "$ARCH"
