...
typedef void (*fnGenUniformGrid2D_fn)(FnNode, float *, float, float, int, int,
                                      float, float, int, float *);
typedef void (*fnGenUniformGrid3D_fn)(FnNode, float *, float, float, float, int,
...
static int find_metadata_id(const FastNoiseApi *api, const char *wanted_name,
                            int *id) {
...
static int find_variable_index(const FastNoiseApi *api, FnNode node,
                               const char *variable_name, int *index) {
...
static int find_node_lookup_index(const FastNoiseApi *api, FnNode node,
                                  const char *lookup_name, int *index) {
  int metadata_id = api->fnGetMetadataID(node);
...
static int find_hybrid_index(const FastNoiseApi *api, FnNode node,
                             const char *hybrid_name, int *index) {
  int metadata_id = api->fnGetMetadataID(node);
...
static int set_variable_float(const FastNoiseApi *api, FnNode node,
...
static int set_variable_int(const FastNoiseApi *api, FnNode node,
...
static int set_node_lookup(const FastNoiseApi *api, FnNode node,
                           const char *lookup_name, FnNode source) {
  int index;
...
static int set_hybrid_float(const FastNoiseApi *api, FnNode node,
                            const char *hybrid_name, float value) {
  int index;
...
static int create_metadata_fbm_simplex(const FastNoiseApi *api,
                                       NodePair *pair) {
  int simplex_id;
  int fractal_id;
  int result;
...
static int create_fixture_graph(const FastNoiseApi *api, const Fixture *fixture,
                                NodePair *pair) {
  memset(pair, 0, sizeof(*pair));
  if (fixture->graph_kind == FixtureGraphEncoded) {
    pair->root =
        api->fnNewFromEncodedNodeTree(fixture->encoded_tree, UINT32_MAX);
    if (pair->root == NULL) {
      fprintf(stderr,
              "FastNoise2 failed to load encoded node tree for fixture '%s'\n",
              fixture->name);
      return 3;
    }
...
static size_t fixture_value_count(const Fixture *fixture) {
  return (size_t)fixture->x_count * (size_t)fixture->y_count *
         (size_t)fixture->z_count;
}
...
static int run_fixture(const FastNoiseApi *api, const Fixture *fixture,
  ...
  if (fixture->dimensions == FixtureDimension2D) {
    api->fnGenUniformGrid2D(pair.root, values, fixture->x_offset,
                            fixture->y_offset, fixture->x_count,
                            fixture->y_count, fixture->x_step, fixture->y_step,
                            fixture->seed, min_max);
  } else {
    api->fnGenUniformGrid3D(pair.root, values, fixture->x_offset,
                            fixture->y_offset, fixture->z_offset,
                            fixture->x_count, fixture->y_count,
                            fixture->z_count, fixture->x_step, fixture->y_step,
                            fixture->z_step, fixture->seed, min_max);
  }
  ...
  printf("  fixture=%s graph=%s root=%s featureSet=%" PRIu32
         " seed=%d dims=%d count=%dx%dx%d\n",
         fixture->name, fixture_graph_kind_name(fixture->graph_kind),
         result->root_metadata_name != NULL ? result->root_metadata_name
                                            : "(null)",
         result->active_feature_set, fixture->seed, (int)fixture->dimensions,
         fixture->x_count, fixture->y_count, fixture->z_count);
  printf("    offset=(%.9g,%.9g,%.9g) step=(%.9g,%.9g,%.9g) min=%.9g max=%.9g "
         "digest=%s expected=%s\n",
         fixture->x_offset, fixture->y_offset, fixture->z_offset,
         fixture->x_step, fixture->y_step, fixture->z_step, result->min_value,
         result->max_value, result->digest, fixture->expected_digest);
  printf("    sample[%llu]=%.9g/0x%08" PRIX32 " sample[%llu]=%.9g/0x%08" PRIX32
         " sample[%llu]=%.9g/0x%08" PRIX32 "\n",
         (unsigned long long)result->sample_indexes[0],
         result->sample_values[0], result->sample_bits[0],
         (unsigned long long)result->sample_indexes[1],
         result->sample_values[1], result->sample_bits[1],
         (unsigned long long)result->sample_indexes[2],
         result->sample_values[2], result->sample_bits[2]);

  if (!result->passed) {
    fprintf(stderr, "digest mismatch for fixture '%s': expected=%s actual=%s\n",
            fixture->name, fixture->expected_digest, result->digest);
  }
...
static void write_sample_json(FILE *file, const FixtureResult *result,
                              int sample_index) {
  fprintf(file, "{\"index\":%llu,\"value\":%.9g,\"bits\":\"0x%08" PRIX32 "\"}",
...
static void write_fixture_json(FILE *file, const FixtureResult *result) {
  ...
  json_write_string(file, result->root_metadata_name != NULL
                              ? result->root_metadata_name
                              : "");
  fprintf(file,
          ",\"activeFeatureSet\":%" PRIu32 ",\"seed\":%d,\"dimensions\":%d",
          result->active_feature_set, fixture->seed, (int)fixture->dimensions);
  fprintf(file, ",\"xCount\":%d,\"yCount\":%d,\"zCount\":%d", fixture->x_count,
          fixture->y_count, fixture->z_count);
  fprintf(file, ",\"xOffset\":%.9g,\"yOffset\":%.9g,\"zOffset\":%.9g",
          fixture->x_offset, fixture->y_offset, fixture->z_offset);
  fprintf(file, ",\"xStep\":%.9g,\"yStep\":%.9g,\"zStep\":%.9g",
          fixture->x_step, fixture->y_step, fixture->z_step);
  fprintf(file, ",\"min\":%.9g,\"max\":%.9g", result->min_value,
          result->max_value);
  fputs(",\"samples\":[", file);
...
static int write_report(const char *report_path, const char *library_path,
                        const char *rid, int metadata_count,
                        const FixtureResult *results, size_t result_count,
                        int mismatches) {
  FILE *file;
  size_t i;
  ...
  fprintf(file,
          ",\"metadataCount\":%d,\"cases\":%d,\"mismatches\":%d,\"fixtures\":[",
          metadata_count, CheckedCases, mismatches);
...
static int run_verification(const FastNoiseApi *api, FixtureResult *results,
                            size_t result_count, int *mismatches) {
...
int main(int argc, char **argv) {
  ...
  if (argc != 4) {
    fprintf(stderr,
            "usage: verify-fastnoise2 <library-path> <report-path> <rid>\n");
    return 1;
  }
  ...
  if (result == 0) {
    ...
    result = run_verification(
        &api, results, sizeof(results) / sizeof(results[0]), &mismatches);
    metadata_count = api.fnGetMetadataCount();
  }
  if (result == 0) {
    result = write_report(argv[2], argv[1], argv[3], metadata_count, results,
                          sizeof(results) / sizeof(results[0]), mismatches);
    if (result == 0)
      printf("  report=%s\n", argv[2]);
  }
