...
internal sealed unsafe partial class FreeTypeDemo
{
    ...
    public void ReportLibraryVersion()
    {
    ...
    public void ReportFaceBasics()
    {
        ...
        Require("FT_Face_Properties", ft.FaceProperties(face, []));
    ...
    public void ReportReferenceCounting()
    {
        var face = pathFace;
        Require("FT_Reference_Face", ft.ReferenceFace(face));
        Require("FT_Done_Face", ft.DoneFace(face));
    ...
    public void ReportOptionalAttachments()
    {
        var face = pathFace;
        ReportOptional("FT_Attach_File", ft.AttachFile(face, fontPath));
        ...
        fixed (byte* path = pathBytes)
        {
            ...
            ReportOptional("FT_Attach_Stream", ft.AttachStream(face, &args));
        }
    }

    /// <summary>Reports whether the face exposes selectable bitmap strikes.</summary>
    public void ReportOptionalBitmapStrike()
    {
        var face = pathFace;
        var faceRec = FreeTypeDrawing.ReadFace(face);
        Console.WriteLine($"FT_FaceRec.num_fixed_sizes: {faceRec.NumFixedSizes}");
        ReportOptional("FT_Select_Size", ft.SelectSize(face, 0));
    }

    /// <summary>Exports the first glyph image loaded from the path-based face.</summary>
    public string ExportNewFaceGlyph()
    {
        var face = pathFace;
        Require("FT_Set_Pixel_Sizes", ft.SetPixelSizes(face, 0, 118));
        Require("FT_Load_Char", ft.LoadChar(face, 'F', FreeTypeDrawing.LoadFlagBits(FtLoadFlags.Render)));
    ...
    public string ExportMemoryFaceGlyph()
    {
        var face = memoryFace;
        Require("FT_Set_Pixel_Sizes", ft.SetPixelSizes(face, 0, 118));
        Require("FT_Load_Char", ft.LoadChar(face, 'M', FreeTypeDrawing.LoadFlagBits(FtLoadFlags.Render)));
    ...
    public string ExportOpenFaceGlyph()
    {
        var face = openFace;
        Require("FT_Set_Pixel_Sizes", ft.SetPixelSizes(face, 0, 118));
        var glyphIndex = ft.GetCharIndex(face, 'O');
        Require("FT_Load_Glyph", ft.LoadGlyph(face, glyphIndex, FtLoadFlags.Render));
    ...
    public string ExportSizingApis()
    {
        var face = pathFace;
        var canvas = new PngCanvas(980, 300, DemoColor.Background);

        Require("FT_Set_Pixel_Sizes", ft.SetPixelSizes(face, 0, 34));
        ...
        Require("FT_Request_Size", ft.RequestSize(face, in request));
    ...
    public string ExportRenderModes()
    {
        var face = pathFace;
        ...
        {
            var x = 36 + i * 176;
            Require("FT_Set_Pixel_Sizes", ft.SetPixelSizes(face, 0, 88));
            Require("FT_Load_Char", ft.LoadChar(face, '@', FreeTypeDrawing.LoadFlagBits(FtLoadFlags.Default)));
            ...
            if (error == 0)
                ...
                ReportOptional($"FT_Render_Glyph {modes[i].Label}", error);

            Require("FT_Set_Pixel_Sizes", ft.SetPixelSizes(face, 0, 20));
    ...
    public string ExportLoadFlags()
    {
        ...
        for (var i = 0; i < flags.Length; i++)
        {
            var x = 40 + i * 200;
            Require("FT_Set_Pixel_Sizes", ft.SetPixelSizes(face, 0, 88));
            Require("FT_Load_Char", ft.LoadChar(face, 'g', FreeTypeDrawing.LoadFlagBits(flags[i].Flags)));
            canvas.DrawGlyph(FreeTypeDrawing.CurrentGlyph(face), x + 54, 154, flags[i].Color);
            Require("FT_Set_Pixel_Sizes", ft.SetPixelSizes(face, 0, 18));
    ...
    public string ExportTransform()
    {
        var face = pathFace;
        var canvas = new PngCanvas(720, 300, DemoColor.Background);
        Require("FT_Set_Pixel_Sizes", ft.SetPixelSizes(face, 0, 112));
        Require("FT_Load_Char", ft.LoadChar(face, 'T', FreeTypeDrawing.LoadFlagBits(FtLoadFlags.Render)));
    ...
    public string ExportKerning()
    {
        var face = pathFace;
        var canvas = new PngCanvas(880, 330, DemoColor.Background);
        var a = ft.GetCharIndex(face, 'A');
        var v = ft.GetCharIndex(face, 'V');
        Require("FT_Get_Kerning", ft.GetKerning(face, a, v, FtKerningMode.KerningDefault, out var kerning));
        Console.WriteLine($"FT_Get_Kerning A/V: {FreeTypeValues.Pixel26Dot6(kerning.X)} px");
        ReportOptional("FT_Get_Track_Kerning", ft.GetTrackKerning(face, FreeTypeValues.Long(12 * 64), 0, out _));

        Require("FT_Set_Pixel_Sizes", ft.SetPixelSizes(face, 0, 76));
        ...
        Require("FT_Set_Pixel_Sizes", ft.SetPixelSizes(face, 0, 19));
    ...
    public string ExportCharmapGrid()
    {
        ...
        Require("FT_Set_Pixel_Sizes", ft.SetPixelSizes(face, 0, 21));
        ...
        {
            Require("FT_Load_Glyph", ft.LoadGlyph(face, glyphs[i], FtLoadFlags.Render));
            var x = 44 + i % 12 * 58;
            var y = 112 + i / 12 * 70;
    ...
    public string ExportNamesAndComposite()
    {
        var face = pathFace;
        ...
        ReportOptional("FT_Get_Glyph_Name", ft.GetGlyphName(face, nameIndex, out var glyphNameText));

        Console.WriteLine($"FT_Get_Name_Index \"A\": {nameIndex}");
        Console.WriteLine($"FT_Get_Glyph_Name: {glyphNameText ?? "(none)"}");

        Require("FT_Set_Pixel_Sizes", ft.SetPixelSizes(face, 0, 104));
        Require("FT_Load_Glyph", ft.LoadGlyph(face, nameIndex, FtLoadFlags.Render));
        var nameGlyph = FreeTypeDrawing.CurrentGlyph(face);

        Require("FT_Load_Char", ft.LoadChar(face, '\u00E9', FreeTypeDrawing.LoadFlagBits(FtLoadFlags.NoRecurse)));
        var compositeSlot = FreeTypeDrawing.ReadGlyphSlot(face);
        if (compositeSlot.NumSubglyphs > 0)
            ReportFirstSubglyph(FreeTypeDrawing.GlyphSlot(face));
        else
            Console.WriteLine("FT_Get_SubGlyph_Info: current font did not expose a decomposed U+00E9 composite");

        Require("FT_Load_Char", ft.LoadChar(face, '\u00E9', FreeTypeDrawing.LoadFlagBits(FtLoadFlags.Render)));
    ...
    public string ExportOutline()
    {
        var face = pathFace;
        Require("FT_Set_Pixel_Sizes", ft.SetPixelSizes(face, 0, 160));
        Require(
            "FT_Load_Char",
    ...
    public string ExportFixedPointAndVector()
    {
        var face = pathFace;
    ...
    public string ExportOutlineGeometry()
    {
        var face = pathFace;
        Require("FT_Set_Pixel_Sizes", ft.SetPixelSizes(face, 0, 160));
        Require(
    ...
    public string ExportGlyphBitmapObjects()
    {
        var face = pathFace;
        ...
        ReportDetachedGlyphBox(face, glyphIndex);
        ...
        try
        {
            ...
            Require("FT_Bitmap_Copy", ft.BitmapCopy(library, &source, &copied));
            Require("FT_Bitmap_Convert", ft.BitmapConvert(library, &source, &converted, alignment: 1));
    ...
    public void ReportVariationSelectors()
    {
        var face = pathFace;
    ...
    private void ReportFirstSubglyph(FtGlyphSlotRec* glyphSlot)
    {
        Require(
