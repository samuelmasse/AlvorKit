...
public sealed class AlvorSenseDemoRenderer : IDisposable
{
    ...
    private static readonly string VertexShaderSource = ReadShader("alvorsense-demo.vert.glsl");

    /// <summary>Fragment shader source used by the rectangle renderer.</summary>
    private static readonly string FragmentShaderSource = ReadShader("alvorsense-demo.frag.glsl");
    ...
    private AlvorSenseDemoRenderer(GlLayer gl, GlProgramHandle program, GlVertexArrayHandle vertexArray, GlBufferHandle vertexBuffer)
    {
        this.gl = gl;
    ...
    public static AlvorSenseDemoRenderer Load(GlLayer gl)
    {
    ...
    public void Render(AlvorSenseDemoState state, float elapsedSeconds, int framebufferWidth, int framebufferHeight)
    {
    ...
    private static void AddBoard(Span<float> vertices, ref int cursor, AlvorSenseDemoState state, float elapsedSeconds)
    {
        ...
        AddRect(vertices, ref cursor, state.PlayerX, state.PlayerY, AlvorSenseDemoState.PlayerSize, AlvorSenseDemoState.PlayerSize, 0.18f, 0.48f, 1f);
    ...
    private static void AddPanel(Span<float> vertices, ref int cursor, AlvorSenseDemoState state)
    {
    ...
    private static float ToNdcX(float x) => x / AlvorSenseDemoState.GameWidth * 2f - 1f;

    /// <summary>Converts a client y coordinate into normalized device coordinates.</summary>
    private static float ToNdcY(float y) => 1f - y / AlvorSenseDemoState.GameHeight * 2f;
    ...
    {
        var path = Path.Combine(ProjectRoot.ResDirectory(typeof(AlvorSenseDemoRenderer)), "shaders", "AlvorKit.Script.AlvorSense.Demo", name);
