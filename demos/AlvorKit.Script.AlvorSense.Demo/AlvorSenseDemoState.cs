...
public sealed class AlvorSenseDemoState
{
    ...
    private void AcceptCharacter(uint codepoint)
    {
        ...
        ReadOnlySpan<char> code = ['K', 'I', 'T'];
        CodeProgress = value == code[CodeProgress] ? CodeProgress + 1 : value == 'K' ? 1 : 0;
        if (CodeProgress > 2)
            CodeProgress = 3;
    }
    ...
    public AlvorSenseDemoResult CreateResult(TimeSpan elapsed) =>
        new()
        {
            Won = Won,
            HasKey = HasKey,
...
public sealed class AlvorSenseDemoResult
{
